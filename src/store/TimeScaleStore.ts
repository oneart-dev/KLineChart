...
export default class TimeScaleStore {
  ...
  private _dateTimeFormat: Intl.DateTimeFormat =
    this._buildDateTimeFormat() as Intl.DateTimeFormat;
   ...
   */
  private readonly _minVisibleBarCount: MinVisibleBarCount = {
    left: 2,
    right: 2,
  };
  ...
  adjustVisibleRange(): void {
    ...
    const maxRightOffsetBarCount =
      barCount - Math.min(this._minVisibleBarCount.left, dataCount);
    ...
    const minRightOffsetBarCount =
      -dataCount + Math.min(this._minVisibleBarCount.right, dataCount);
  ...
  setBarSpace(barSpace: number, adjustBeforeFunc?: () => void): void {
    if (
      barSpace < BarSpaceLimitContants.MIN ||
  ...
  getDataByDataIndex(dataIndex: number): Nullable<KLineData> {
    ...
    if (dataIndex >= length) {
      ...
      const beforeLast = dataList[length - 2];
      const diff = last.timestamp - beforeLast.timestamp;
      const newTimestamp = last.timestamp + diff * (dataIndex - length + 1);
      return {
  ...
  timestampToDataIndex(timestamp: number): number {
    ...
    if (timestamp >= dataList[length - 1].timestamp) {
      ...
      const width = (timestamp - last.timestamp) / diff;
  ...
  zoom(scale: number, coordinate?: Partial<Coordinate>): void {
    ...
    this.setBarSpace(barSpace, () => {
      this._offsetRightBarCount +=
        floatIndex - this.coordinateToFloatIndex(coordinate?.x as number);
    });
  }
